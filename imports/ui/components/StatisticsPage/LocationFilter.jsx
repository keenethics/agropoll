  ...
  renderSelect(name, title, items) {
    return (
      <div className="float-left filter-select-container">
        <select className="select-filter" name={name} onChange={this.selectAdmLevel} value={this.props.administrative_area_level_1 || 'none'}>
    ...
    return (
      <div className="LocationFilter-wrapper percent-100 float-left">
        {this.renderSelect(
          ...
          { ua: 'Вся Україна', en: 'Whole Ukraine' }[localStorage.getItem('language') || 'ua'],
          'admLevel1Items'
        )}
        {/*
        {this.props.administrative_area_level_1 ? this.renderSelect('administrative_area_level_2', 'Вся область', 'admLevel2Items') : ''}
        {this.props.administrative_area_level_2 ? this.renderSelect('place_id', 'Весь район', 'places') : ''}
        */}
      </div>
    );
  }
